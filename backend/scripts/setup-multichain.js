...
async function setupMultiChain() {
  ...
  try {
    ...
    console.log('3. Run: node scripts/migrate-audit-logs.js (if migrating)');
    ...
    console.log('4. Run fix script: ./scripts/fix-multichain.sh');
    process.exit(1);
  }
}
...
async function createStreams() {
  console.log('\n📝 Creating streams...');
  const streams = ['audit_logs', 'patient_records', 'appointments', 'lab_results'];
...
async function grantPermissions() {
  ...
  try {
    await executeCommand(
      `multichain-cli ${chainName} grant ${defaultUser} connect,send,receive,create,issue,mine,write`,
...
module.exports = { setupMultiChain };