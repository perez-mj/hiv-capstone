...
class Queue {
  static async generateQueueCode(queueNumber, queueStream = null) {
  // If stream is provided, use stream-specific prefix
  if (queueStream === 'TESTING') {
    return `T-${String(queueNumber).padStart(3, '0')}`;
  } else if (queueStream === 'CONSULTATION') {
    return `C-${String(queueNumber).padStart(3, '0')}`;
  }
  ...
  // NEW: Get current queue organized by stream (Testing vs Consultation)
  static async getCurrentQueueByStream() {
    try {
      // Get all active service types
      const [serviceTypes] = await pool.execute(
        `SELECT id, type_name, duration_minutes 
         FROM appointment_types 
         WHERE is_active = 1 
         ORDER BY type_name`
      );

      // Get all active queue entries with appointment details, grouped by stream
      const [queues] = await pool.execute(
        ...
        ORDER BY 
          q.queue_stream,
          q.priority DESC,
          q.stream_queue_number ASC`
      );

      // Separate into Testing and Consultation streams
      const testingStream = {
        stream: 'TESTING',
        waiting: [],
        called: [],
      ...
      // Distribute queues to their respective streams
      for (const queue of queues) {
        if (queue.queue_stream === 'TESTING') {
          if (queue.status === 'WAITING') {
            testingStream.waiting.push(queue);
          } else if (queue.status === 'CALLED') {
            testingStream.called.push(queue);
          } else if (queue.status === 'SERVING') {
            testingStream.serving.push(queue);
            if (!testingStream.now_serving) {
              testingStream.now_serving = queue;
            }
          }
          testingStream.total++;
        } else {
          if (queue.status === 'WAITING') {
            consultationStream.waiting.push(queue);
          } else if (queue.status === 'CALLED') {
            consultationStream.called.push(queue);
          } else if (queue.status === 'SERVING') {
            consultationStream.serving.push(queue);
            if (!consultationStream.now_serving) {
              consultationStream.now_serving = queue;
            }
          }
          consultationStream.total++;
        }
      }

      // Calculate waiting times (average 15 minutes per patient)
      const avgServiceTime = 15;
      testingStream.estimated_wait = testingStream.waiting.length * avgServiceTime;
      consultationStream.estimated_wait = consultationStream.waiting.length * avgServiceTime;

      return {
        testing: testingStream,
        consultation: consultationStream,
        stats: {
          total_waiting: testingStream.waiting.length + consultationStream.waiting.length,
          total_called: testingStream.called.length + consultationStream.called.length,
          total_serving: testingStream.serving.length + consultationStream.serving.length,
          testing_waiting: testingStream.waiting.length,
          consultation_waiting: consultationStream.waiting.length,
          testing_serving: testingStream.serving.length,
          consultation_serving: consultationStream.serving.length,
          total_in_queue: queues.length
        }
      };
    } catch (error) {
      console.error('Error in getCurrentQueueByStream:', error);
      throw error;
    }
  }

  // NEW: Get next waiting patient for specific stream
  static async getNextWaitingPatientForStream(stream) {
    const [rows] = await pool.execute(
      ...
      WHERE DATE(q.created_at) = CURDATE()
        ...
        AND q.queue_stream = ?
      ORDER BY q.priority DESC, q.stream_queue_number ASC
      LIMIT 1`,
      [stream]
    );
    return rows[0];
  ...
  // NEW: Get patient's stream position
  static async getPatientPositionInStream(streamQueueNumber, stream) {
    const [rows] = await pool.execute(
      `SELECT COUNT(*) + 1 as position
       FROM queue q
       WHERE DATE(q.created_at) = CURDATE()
         AND q.status IN ('WAITING', 'CALLED')
         AND q.stream_queue_number < ?
         AND q.queue_stream = ?`,
      [streamQueueNumber, stream]
    );
  ...
  // NEW: Get next stream queue number
  static async getNextStreamQueueNumber(stream) {
    const [lastQueue] = await pool.execute(
      `SELECT MAX(stream_queue_number) as max_num 
       FROM queue 
       WHERE DATE(created_at) = CURDATE() AND queue_stream = ?`,
      [stream]
    );
    return (lastQueue[0].max_num || 0) + 1;
  }
  ...
  const [result] = await pool.execute(
    `INSERT INTO queue (
      ...
      created_by, created_at, updated_at, queue_stream, stream_queue_number
    ) VALUES (?, ?, ?, ?, 'WAITING', ?, NOW(), NOW(), ?, ?)`,
    [appointment_id, queue_number, queue_code, priority || 0, created_by, queue_stream, stream_queue_number]
  );
  
  return result.insertId;
}

  // Create for appointment with stream assignment
  ...
  try {
    ...
    // Determine stream based on appointment type
    const typeName = appointment[0].type_name?.toLowerCase() || '';
    const queueStream = typeName === 'testing' ? 'TESTING' : 'CONSULTATION';
    
    // Get stream-specific queue number
    const streamQueueNumber = await this.getNextStreamQueueNumber(queueStream);
    const globalQueueNumber = await this.getNextQueueNumber();
    
    // Generate stream-specific queue code
    const queueCode = await this.generateQueueCode(streamQueueNumber, queueStream);
  ...
  static async findByAppointmentId(appointmentId) {
    const [rows] = await pool.execute(
      `SELECT q.*, q.queue_code, q.queue_stream, a.appointment_type_id
       FROM queue q
       JOIN appointments a ON q.appointment_id = a.id
  ...
  // Check appointment in queue
  static async checkAppointmentInQueue(appointmentId) {
    try {
      const [rows] = await pool.execute(
        `SELECT q.id, q.queue_number, q.queue_code, q.status, q.queue_stream, q.stream_queue_number, a.appointment_type_id
         FROM queue q
         JOIN appointments a ON q.appointment_id = a.id
         WHERE q.appointment_id = ? AND DATE(q.created_at) = CURDATE()`,
        [appointmentId]
      );
      ...
      if (rows.length > 0) {
        return {
          ...
          queue_stream: rows[0].queue_stream,
          stream_queue_number: rows[0].stream_queue_number,
  ...
  // Get current queue optimized (legacy method for backward compatibility)
  ...
  static async getQueueDisplay() {
    ...
    for (const row of rows) {
      const stream = row.queue_stream || 'CONSULTATION';
      if (row.status === 'WAITING') groupedByStream[stream].waiting.push(row);
      else if (row.status === 'CALLED') groupedByStream[stream].called.push(row);
      else if (row.status === 'SERVING') groupedByStream[stream].serving.push(row);
    }
    
    return groupedByStream;
  }

  // Get queue statistics by service type
  ...
  static async getQueueHistory(filters = {}, pagination = {}) {
    const { start_date, end_date, status, service_type_id, stream } = filters;
      ...
      `SELECT 
        ...
        (SELECT COUNT(*) + 1 
         ...
         WHERE DATE(q2.created_at) = CURDATE() 
           ...
           AND q2.stream_queue_number < q.stream_queue_number
           AND q2.queue_stream = q.queue_stream) as position
      FROM queue q
      JOIN appointments a ON q.appointment_id = a.id
      LEFT JOIN appointment_types at ON a.appointment_type_id = at.id
      WHERE a.patient_id = ?
  ...
  // Get next waiting patient (legacy)
  ...
  static async getQueueEntriesForReset(filters = {}) {
    const { appointment_id, date } = filters;
    let query = `
      SELECT q.*, a.patient_id, p.first_name, p.last_name, a.appointment_type_id
      FROM queue q
      JOIN appointments a ON q.appointment_id = a.id
      JOIN patients p ON a.patient_id = p.id
      WHERE 1=1
